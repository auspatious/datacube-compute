...
@pytest.fixture
def arr(kwargs):
    xx = np.random.randint(0, 10_000, size=(100, 100, 10, 50), dtype=np.uint16)
...
@pytest.fixture
def kwargs(): 
    return {"maxiters": 1000, "eps": 1e-5, "num_threads": 1, "nodata": 11_111, "scale": 1 / 10_000}


@pytest.fixture
def kwargs_parallel(): 
    return {"maxiters": 1000, "eps": 1e-5, "num_threads": 4, "nodata": 11_111, "scale": 1 / 10_000}


def test_benchmark_uint16(benchmark, arr, kwargs):
    benchmark(geomedian, arr, **kwargs)


def test_benchmark_parallel_uint16(benchmark, arr, kwargs_parallel):
